...
// Consulta para obtener solo los mangas
$datos = $conexion->query("SELECT * FROM comics WHERE tipo = 'manga'");

// Verificar si se seleccionó un género
$genero = isset($_GET['genero']) ? $conexion->real_escape_string($_GET['genero']) : "";

// Construir la consulta SQL
$sql = "SELECT * FROM comics WHERE tipo = 'manga'";
if (!empty($genero)) {
...
<head>
    ...
    <title>Comic House - Mangas</title>
...
<header>
    ...
    <div class="Comics-titulo-2">
        <div class="titulo-comics-ctn-2">
            <h2>Catálogo de Mangas<?= !empty($genero) ? " - " . htmlspecialchars($genero) : "" ?></h2>
        </div>
    </div>

    <div class="Comics-base-2">
        <div class="Comics-container-2">
            <?php while ($manga = $datos->fetch_object()) { ?>
                <div class="Comics-btn-2">
                    <button class="Comics-btns-2" onclick="location.href='../Comics/comics.php?id=<?= $manga->id ?>'"
                        style="background-image: url('../uploads/<?= htmlspecialchars($manga->portada) ?>');">
                    </button>
                    <div class="Comics-info-2">
                        <p class="title-c-2"><?= htmlspecialchars($manga->titulo) ?></p>
                        <p class="description-c-2"><span><?= htmlspecialchars($manga->clasificacion) ?></span>
                            n°.<?= htmlspecialchars($manga->capitulos) ?></p>
                    </div>
                </div>
            <?php } ?>
        </div>
